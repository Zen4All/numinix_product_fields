...
if (zen_not_null($_POST))
{
	...
	$products_thumbnail    = new upload('products_thumbnail');
	$products_thumbnail->set_destination(DIR_FS_CATALOG_IMAGES . $_POST['img_dir']);
	if ($products_thumbnail->parse() && $products_thumbnail->save(true))
	{
		$products_thumbnail_image_name = 'thumbnails/' . $products_thumbnail->filename;
	}
}
else
{
...
?>
	...
	echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . zen_output_string_protected($pInfo->products_name);
?></td>
...
?>
      ...
      <tr>
        <td align="right"><?php
	echo '<a href="' . zen_href_link($back_url, $back_url_params . (isset($_POST['search']) ? '&search=' . $_POST['search'] : ''), 'NONSSL') . '">' . zen_image_button('button_back.gif', IMAGE_BACK) . '</a>';
?></td>
